...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        utils = StatisticsUtils()
        datos = [1, 2, 3, 4]
        result_esperado = [1.5, 2.5, 3.5]
        result = utils.moving_average(datos, window = 2)
        npt.assert_allclose(result, result_esperado)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        with self.assertRaises(ValueError):
            utils.moving_average(datos, window = 0)
        with self.assertRaises(ValueError):
            utils.moving_average(datos, window = 4)
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        datos = [[1, 2], [3, 4]]
        with self.assertRaises(ValueError):
            utils.moving_average(datos, window= 2)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        utils = StatisticsUtils()
        datos = [2, 4, 6]
        datos_esperados = [0.0, 0.5, 1.0]
        result = utils.min_max_scale(datos)
        self.assertAlmostEqual(np.min(result), 0.0)
        self.assertAlmostEqual(np.min(result), 1.0)
        npt.assert_allclose(result, datos_esperados, rtol=1e-10, atol=1e-10)
